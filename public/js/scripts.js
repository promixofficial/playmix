...
/***/ function(module, exports, __webpack_require__) {
	...
	angular.module('playMixApp', ['ngMaterial', 'angular-sortable-view', 'angularResizable', 'LocalForageModule']).constant('YT_event', {
	  STOP: 0,
	  PLAY: 1,
	  PAUSE: 2,
	  STATUS_CHANGE: 3,
	...
	angular.module("playMixApp").factory('utilsFct', [function () {
	    ...
	    var Utils = {
	        Time: {
	            ...
	            YTTime: {
	                PTToSeconds: function PTToSeconds(ptTime) {
	    ...
	    var Search = Object.defineProperties({
	        ...
	        fetch: function fetch(callback) {
	            ...
	            if (this.searchText !== "" && this.searchText !== this.lastSearch) {
	                ...
	                $http.get(this.searchUrl).then(function (response) {
	        ...
	        getVideos: function getVideos(callback) {
	            $http.get(this.videoListUrl).then(function (response) {
	...
	angular.module("playMixApp").factory('listFct', ['storageFct', function (storageFct) {
	  ...
	  var List = Object.defineProperties({
	    ...
	    addList: function addList() {
	      this.playlists.unshift({ name: 'List ' + (this.playlists.length + 1), id: new Date().getTime(), playlist: [] });
	      this.onChangeList();
	    },
	    removeList: function removeList(list) {
	      var index = this.playlists.indexOf(list);
	      this.playlists.splice(index, 1);
	      storageFct.removeItem('_playlist_' + list.id);
	      this.onChangeList();
	    },
	    ...
	    isEditing: function isEditing(list) {
	      return this.editingList === list;
	    },
	    setLastSelectedListIndex: function setLastSelectedListIndex() {
	      var lastIndex = this.selectedIndex;
	      storageFct.setItem('lastSelectedListIndex', lastIndex !== -1 ? lastIndex : 0);
	    },
	
	    onChangeList: function onChangeList(list) {
	      if (list) {
	        this.saveList(list);
	      } else {
	        this.saveLists();
...
/***/ function(module, exports) {
	...
	angular.module("playMixApp").factory('playlistFct', ['$rootScope', '$timeout', 'YT_event', 'utilsFct', 'storageFct', function ($rootScope, $timeout, YT_event, utilsFct, storageFct) {
	  ...
	  var Playlist = Object.defineProperties({
	    ...
	    play: function play(position) {
	      ...
	      if (this.listLength) {
	        ...
	        $timeout(function () {
	          _this.sendControlEvent(YT_event.PLAY);
	        }, 300);
	      }
	    },
	    ...
	    setLastPlayedIndex: function setLastPlayedIndex() {
	      var lastIndex = this.selectedIndex;
	    ...
	    concatPlaylist: function concatPlaylist(playlist) {
	      if (this.listLength) {
	        var that = this;
	        this.clearIdIndex();
	    ...
	    setIdIndex: function setIdIndex() {
	      var _this2 = this;
	
	      this.clearIdIndex();
	      this.list.forEach(function (item) {
	        _this2.idIndex[item.id] = true;
	      });
	    },
	    clearIdIndex: function clearIdIndex() {
	      this.idIndex = {};
	    },
	  ...
	  var PlaylistSection = Object.defineProperties({
	    ...
	    scrollToVideo: function scrollToVideo(videoIndex) {
...
/***/ function(module, exports) {
	...
	angular.module("playMixApp").directive('youtubePlayer', ['$window', 'YT_event', function ($window, YT_event) {
	  return {
	    ...
	    link: function link(scope, element, attrs, $rootScope) {
...
/***/ function(module, exports) {
	...
	angular.module("playMixApp").controller('mainCtrl', function ($scope, $rootScope, $timeout, $mdSidenav, $log, YT_event, listFct, playlistFct, searchFct, $localForage) {
